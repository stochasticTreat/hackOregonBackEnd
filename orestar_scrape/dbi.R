...
dbiWrite<-function(tabla, 
  ...
  con <- dbConnect(drv, dbname=dbname, host='localhost', password="points", 
  ...
  if(length(res)){
  	if(sum(grepl(pattern="error", x=class(res), ignore.case=T))){
  		message("\nError encountered in transmission to database\n")
  		dbDisconnect(con)
  		return(res)
  	}
  	cat("\nTable written to database found in database:")
...
dbTableExists<-function(tableName, dbname="interactome", port=5432){
	library("DBI")
	library("RPostgreSQL")
	drv <- dbDriver("PostgreSQL")
	con <- dbConnect(drv, dbname=dbname, host='localhost', password="points", port=port, user="postgres")
	res = dbExistsTable(conn=con, name=tableName)
	dbDisconnect(con)
	return(res)
}
